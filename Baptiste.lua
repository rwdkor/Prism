...
local Baptiste_First_Init = true
...
local HeroDatabase = {
    ...
    ["Baptiste"] = { HeroID = 0x02E0000000000029, UltMax = 1680, NPC = false, Resource },
    ...
    ["Baptiste"] = { HeroID = 0x02E0000000000003, UltMax = 1260, NPC = false, Resource },
    ...
    ["Baptiste"] = { HeroID = 0x02E0000000000020, UltMax = 2310, NPC = false, Resource },
...
local SkillDatabase = {
    ...
    ["Baptiste"] = { 
        ["BaptisteShift"] = { Resource, slot = 1, type = 4, charging = false, casting = false, id = 0x3D, extra_id = 0x6 },
        ["BaptisteE"] = { Resource, slot = 2, type = 5, charging = false, casting = false, id = 0x3D, extra_id = 0x7 },
        ["BaptisteUlt"] = { Resource, slot = 5, type = 2, charging = false, casting = false, id = 0x1510, extra_id = 0 },
    },
    ["Hana"] = { 
    ...
    ["Baptiste"] = { 
        ["BaptisteShift"] = { Resource, slot = 1, type = 5, charging = false, casting = false, id = 0x3D, extra_id = 0x2 },
        ["BaptisteE"] = { Resource, slot = 2, type = 4, charging = false, casting = false, id = 0x3D, extra_id = 0x1 },
        ["BaptisteUlt"] = { Resource, slot = 5, type = 2, charging = false, casting = true, id = 0, extra_id = 0 },
    },
    ["Widowmaker"] = { 
...
function Baptiste:TargetSelector(Mode)
    ...
    for i = 0, Count - 1 do
        ...
        if (Health and (HealthCurrent > 0) and Player:IsEnemy()) then
            ...
            if (Baptiste.BestTarget == nil) then
                local Found = Game.Engine:FindBestBone(Player, Baptiste.BestFov, 0x11, 1)

                if (Found.bFound) then
                    Baptiste.BestTarget = Player
                    Baptiste.BestFov = Found.lastFov
                    Baptiste.BestBone = Found.foundBone
                end
                goto Continue
            end

            if (Mode == 0) then --Closest
                if (dst(LocalPlayer:GetMesh():GetLocation(), Baptiste.BestTarget:GetMesh():GetLocation()) >=
                    dst(LocalPlayer:GetMesh():GetLocation(), Player:GetMesh():GetLocation())) then
                    Baptiste.BestTarget = Player
                end
            elseif (Mode == 1) then --LowestHP
                if (Baptiste.BestTarget:GetHealth():GetLife().y >= HealthCurrent) then
                    Baptiste.BestTarget = Player
                end
            ...
            elseif (Mode == 3) then --ClosestAim
                local LocalPos = Game.Engine:GetViewMatrix():GetCameraVec()
                local Control = Game.Engine:GetController()
                if (CalcFovFromLocalAngle(CalcAngleFromLocalPosition(Baptiste.BestTarget:GetMesh():GetBonePos(Baptiste.BestTarget:GetBoneId(1)))) >=
            ...
            elseif (Mode == 4) then --Smart
                local _Friends = Baptiste:NearFriends(Baptiste.BestTarget, true)
                local Friends = Baptiste:NearFriends(Player, true)
                local _Distance = dst(LocalPlayer:GetMesh():GetLocation(), Baptiste.BestTarget:GetMesh():GetLocation())
                local Distance = dst(LocalPlayer:GetMesh():GetLocation(), Player:GetMesh():GetLocation())
                local _HealthCurrent = Baptiste.BestTarget:GetHealth():GetLife().y

                local _Score = Baptiste:CalcTargetScore(_Friends, _Distance, _HealthCurrent)
                local Score = Baptiste:CalcTargetScore(Friends, Distance, HealthCurrent)
...
--[[
    Baptiste:CalcTargetScore
--]]
function Baptiste:CalcTargetScore(Friends, Distance, HealthCurrent)
    return Friends + Distance / 10 + HealthCurrent / 100
end

--[[
    Baptiste:NearFriends
    Enemy - True: Enemy, False: Team
--]]
function Baptiste:NearFriends(_Player, Enemy)
    local Count = Game.Engine:GetPlayerCount()
...
--[[
    Baptiste:UsingDragonblade
--]]
function Baptiste:NanoBoosted()
...
--[[
    Baptiste:Aimbot
--]]
function Baptiste:Aimbot(Speed, Fov_Free)
	if (Baptiste.MeleeAimbot) then
        local Mesh = Baptiste.MeleeTarget:GetMesh()
            if (Mesh) then
                ...
                local AimSpeed = Baptiste.MeleeAimbotSpeed
                local Control = Game.Engine:GetController()
                local AngleDiff = CalcFovFromLocalAngle(CalcAngleFromLocalPosition(PredictPos))

                if (Baptiste.UsingDragonblade() == 0) then
    ...
    elseif (Game.Utils.IsKeyPressed(0x1)) then
        if (Baptiste.BestBone ~= -1) then
            local Mesh = Baptiste.BestTarget:GetMesh()
            if (Mesh) then
                local locPos = Game.Engine:GetViewMatrix():GetCameraVec()
                local bonePos = Mesh:GetBonePos(Baptiste.BestBone)

                local PredictPos = Math.Predict(locPos, bonePos, Mesh:GetVelocity(), 175, 0)
    ...
    for i = 0, Count - 1 do
        ...
        if (Health and (HealthCurrent > 0) and Player:IsEnemy()) then
			...
			if (Fov_Enemy_Local > (0.6^Distance)*1.5) then
				Baptiste.MeleeAimbot = true
				Baptiste.MeleeTarget = Player
				Baptiste.MeleeAimbotSpeed = 0.025
				return
			else
				Baptiste.MeleeAimbot = true
				Baptiste.MeleeTarget = Player
				Baptiste.MeleeAimbotSpeed = 0.001
...
--[[
    Baptiste:OnUpdate
--]]
function Baptiste:OnUpdate()
	Baptiste:TargetSelector(3)

    if (Baptiste.BestTarget ~= nil) and (Baptiste.BestBone == -1) then
        local Found = Game.Engine:FindBestBone(Baptiste.BestTarget, Game.Engine:GetFov(), 0x11, 1)
...
function OnUpdate()
    ...
    Baptiste:OnUpdate()
	Baptiste:AutoMelee()
    Baptiste:Aimbot(0.012, 0.009)
end

function OnDraw()
    collectgarbage("collect")
    if (Baptiste_First_Init) then
        Game.Engine:SetFov(Global_Fov)
        Baptiste_First_Init = false;
    end
end
