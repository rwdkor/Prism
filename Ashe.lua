...
function adm()
	...
	if (HeroId ~= 0x02E0000000000200) or (bflick == true) then
		return false
	end
	
	local ctr = Game.Engine:GetController()
	local tcnt = Game.Engine:GetTriggersCount()
	for i=0, tcnt-1 do
		local pt = Game.Engine:GetTriggersAt(i)
		if (pt) and (pt:GetTriggersId() == 9523) then
			local out = Math.XMFLOAT2(0, 0)
			local m = pt:GetMesh()
			if (m) then bflick = true
				local b1 = 0.0 local b2 = 0.0
				local lp = Game.Engine:GetViewMatrix():GetCameraVec()
				local tp = m:GetLocation()
				local rr = Game.Engine:RayCast(lp, tp, 4)
				if (rr.bHitted) then return end
				if (Game.Engine:WorldToScreen(tp, out)) and not (pt:IsEnemy()) then
				for i=10,1,-1 do b1 = b1 + (0.01 * i / 10) if b1 > 1.0 then b1 = 1.0 end
				local lp = Game.Engine:GetViewMatrix():GetCameraVec()
				local tp = m:GetLocation()
				local bez1 = curve({tp.x, lp.x},{lp.y, tp.y},{lp.z, tp.z})
				x1, y1, z1 = bez1(0.43) local xm = Math.XMFLOAT3(x1+(0.1*math.random(-1,1)),y1+(0.1*math.random(-1,1)),z1+(0.1*math.random(-1,1)))
				local cAng = Math.LerpVec3(ctr:GetAngle(), Math.NormalizeVec3(xm - lp), b1)
				ctr:SetAngle(cAng) sleep(0.006)
				end
				for i=1,10,1 do b2 = b2 + (0.007 * i) if b2 > 1.0 then b2 = 1.0 end	
				local lp = Game.Engine:GetViewMatrix():GetCameraVec() 
				local tp = m:GetLocation()
				local bez2 = curve({tp.x, x1},{y1, tp.y},{z1, tp.z})
				x2, y2, z2 = bez2(0.57) local xm = Math.XMFLOAT3(x2,y2,z2)
				local cAng = Math.LerpVec3(ctr:GetAngle(), Math.NormalizeVec3(xm - lp), b2)
				ctr:SetAngle(cAng) sleep(0.005)
				end
				ctr:SetKeyCode(1)
				return true
				end
...
function OnDraw()
    collectgarbage("collect")
    Game.Renderer:DrawText("Curved Ashe", 10, 20, 15, 0xFF505050, false)
end
