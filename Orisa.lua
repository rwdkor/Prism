	...
	if (Game.Utils.IsKeyPressed(0x1)) then --Only supports Left Key
		if (bestBone > 0) then
			...
			if (Mesh) then
				...
				local predicted = Math.Predict(locPos, bonePos, Mesh:GetVelocity(), 80, 0)
				predicted.y = bonePos.y + (predicted.y - bonePos.y ) * 0.5
...
function OnDraw()
    collectgarbage("collect")
    Game.Renderer:DrawText("UdyrPack - Zenyatta", 10, 20, 16, 0xFFFFFFFF, false)
end
